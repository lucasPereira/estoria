...
public class EscoltadorDeTestesPadrao extends Runner implements Filterable {

	private SuiteDeTeste suiteDeTeste;
	private RunNotifier mensageiroDeEscolta;

	public EscoltadorDeTestesPadrao(SuiteDeTeste suiteDeTeste, Ata ata) {
	...
	private void executar() {
		...
		List<Metodo> testesExecutados = new LinkedList<>();
		...
		ramosDeExecucao.forEach(classeDeTeste -> {
			...
			executar(classeDeTeste, testes, testesExecutados, invocadores);
		});
		testes.forEach(metodoDeTeste -> {
			Map<ClasseDeTeste, InvocadorDeMetodo<?>> invocadores = new HashMap<>();
			executar(metodoDeTeste.obterClasseDeTeste(), testes, testesExecutados, invocadores);
		});
	}

	private void executar(ClasseDeTeste classeDeTeste, List<Metodo> testes, List<Metodo> testesExecutados, Map<ClasseDeTeste, InvocadorDeMetodo<?>> invocadores) {
		...
		classeDeTeste.obterClassesProvedoras().forEach(classeProvedora -> {
			executar(classeProvedora, testes, testesExecutados, invocadores);
		});
		...
		classeDeTeste.obterMetodosDeTeste().forEach(metodoDeTeste -> {
			executar(metodoDeTeste, testes, testesExecutados, invocadores);
		});
	}

	private void executar(Metodo metodoDeTeste, List<Metodo> testes, List<Metodo> testesExecutados, Map<ClasseDeTeste, InvocadorDeMetodo<?>> invocadores) {
		if (!testesExecutados.contains(metodoDeTeste)) {
			...
			testesExecutados.add(metodoDeTeste);
		}
	}
